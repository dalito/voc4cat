...
[group('individual steps')]
join: _fake_actions_env
  @voc4cat transform --config _main_branch/idranges.toml --logfile outbox/voc4cat.log -O outbox --join vocabularies/

# Add provenance information to all ttl files in vocabularies/
[group('individual steps')]
prov: _fake_actions_env
  voc4cat transform --prov-from-git --config _main_branch/idranges.toml --inplace --logfile outbox/voc4cat.log vocabularies/
